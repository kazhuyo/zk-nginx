...
### Build
```docker
docker build --build-arg "NGINX_VERSION=1.15.5" -t kazhuyo/zk-nginx .
```
Replace `NGINX_VERSION=1.17.0` with your preferred version. You can also simply `pull` the images. See below.
### Pull
```docker
docker pull kazhuyo/zk-nginx:latest
```

You can also use a different version of NGINX simply by pulling a build with the NGINX version you want. For example;
```docker
docker pull kazhuyo/zk-nginx:latest
docker pull kazhuyo/zk-nginx:1.15.4
docker pull kazhuyo/zk-nginx:1.15.3
```
To see the available versions vist https://hub.docker.com/r/kazhuyo/zk-nginx/tags/
...
```
 ...
 Check out the high performance docker PHP service we published here: [kazhuyo/zk-phpfpm](https://github.com/kazhuyo/zk-phpfpm)

Server locations are define via `upstream`:
```nginx
upstream php-fpm {include /etc/nginx/upstream.d/php-fpm.conf;}
```
While you can pair this with any php-fpm service, it is recommended that you use this highly optimized [PHP-FPM Docker Service](https://github.com/kazhuyo/zk-phpfpm). Most of our `docker-compose` files will reference this.
...
* Generate your certificate: `/usr/local/bin/certbot-auto certonly -n --debug --agree-tos --email bob@gmail.com --standalone -d *.example.com`
...
59 4,16 * * * /usr/local/bin/certbot certonly -n --debug --agree-tos --pre-hook="docker stop nginx" --post-hook="docker start nginx" --standalone -d *.example.com > /dev/null
EOF
```
...
# Permissions
We had standardized on the user, group and UID/GID to work seamlessly with other applications [like PHP-FPM](https://hub.docker.com/r/kazhuyo/zk-phpfpm/)
...
## `PageSpeed` Benchmark
This test was run using PageSpeed. This was running a full Wordpress stack and the default WP theme.
...
To see the available versions visit: https://hub.docker.com/r/kazhuyo/zk-nginx/tags/
